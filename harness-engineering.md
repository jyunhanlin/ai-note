...
- **生成 / 評估分離**:**模型對自己的產出有正向偏誤**（positive bias）——它寫的程式碼，自己 review 容易說「沒問題」。把生成 agent 與評估 agent 拆成不同實例（各自獨立 context、甚至不同 system prompt），評估才有獨立判斷。這是面向 4「分離」原則用在驗證上的一個 instance
