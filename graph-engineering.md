...
來源是一則 X 長貼文（Codez, 2026-07-20），**二手材料**，判讀方式與逐條校準見 §十。

### 這些 code 是什麼

本篇所有 code 都是 **Claude Code dynamic workflow 腳本**：一段純 JavaScript，由 Claude 寫、由 runtime 直接執行，不是又一輪對話。

- `agent()` —— 派出一個 subagent
- `parallel()`／`pipeline()` —— 兩個併發原語
- `schema`／`phase`／`isolation`／`model`／`effort` —— `agent()` 的選項
- `log()`／`budget` —— 全域

你不能自己 `node script.js` 把它跑起來。怎麼讓它開起來見 §七，授權規則見 §八 #14，改完腳本重跑見 §八 #8。

### 先講適用邊界

**本篇假設你已經知道要做什麼。** 範圍沒結晶化的時候，把它攤成圖只會讓你更快、更平行地做錯事。

這是 [harness-engineering.md](./harness-engineering.md) §六 記錄的社群實戰結論：workflow 翻車主因是「範圍沒結晶化」，不是圖畫得不好。形狀問題與規格問題是兩件事，這篇只解前者。

### 在 repo 裡的位置

本 repo 有四篇「分層」筆記，分工是同一件事的四個層：
...
（另有 [ihower-harness-engineering.md](./ihower-harness-engineering.md) 的開發者視角與 [boris-cherny-tips.md](./boris-cherny-tips.md) 的實務技巧，不在這條軸上。）

這篇不是第五層，是 harness §四「面向 4：Workflow 編排」的深潛——面向 4 獨佔的問題是「什麼順序、拆給誰做？」。放成獨立一篇的理由只有一個：面向 4 要照顧五面向的平衡，塞不下拓樸決策，而拓樸決策恰好是實際跑起來最會痛的地方。

**注意術語接縫**：面向 4 把這個拓樸叫「Fork-Join 並行」（用 `git worktree` 實作），本篇拆成 fan-out／fan-in／diamond／pipeline 四型。至於 workflow 這個 primitive 該不該選，在 **harness §六**的四 primitive 表，不在這裡。
...
攤開之後買到的是**獨立性**（可同時跑）與**故障圍堵**（§六 6.1），**不是冗餘**。這個區分要抓緊：「C 卡住 D 就永遠不跑」是**串連耦合**的症狀，跟有沒有備援無關。（原文把這裡寫成「鏈沒有冗餘所以脆弱」，校正見 §八 #5。）

冗餘得另外加，而且有三種不同的加法，別混為一談：

- **majority voting**（self-consistency）——同一個生產 node 跑 N 次再取多數
- **best-of-N**——產 N 個候選再用評審挑最好的，就是 §五 5.1 的 judge panel
- **adversarial verify**——替產出配 N 個獨立質疑者（§五 5.1）

前兩者買的是產出品質，第三者買的是誤報過濾。三者都是額外掛上去的，不是圖天生就有。
...
第 3 點決定你的預期值：**fan-out 的收益是 min(N, C) 倍，不是 N 倍**。N 遠大於 C 時，wall-clock 粗估是 `ceil(N/C) × 單項耗時`——這是均一耗時下的下界，項目快慢差很多時每一波還會被最慢的那個拖住。丟 100 個 thunk 進去它們都會完成，但是分批完成的。
...
> 這件事的完整版都在 [harness-engineering.md](./harness-engineering.md)：§四 面向 1 的 tool-call offloading，以及該篇 §六「中間結果落點」（原話：「面向 1 offloading 開到極致」）。
...
下面的範例會用到一個 `judgeAll(items)`，這裡只需要它的契約，不需要它的內部：**收一批發現，回傳同樣一批、每筆帶一個 `verdict`（`real`／`rejected`／`unresolved`），保證不含 `null`。** 為什麼會有 `unresolved` 這個第三種結果、門檻怎麼定，是 §五 5.2 的題目。
...
while (dry < 2 && rounds++ < 8 && budget.remaining() > 50_000) {
  ...
  for (const b of await judgeAll(fresh)) {           // 契約如上：每筆都帶 verdict，無 null
...
**功勞要歸對地方**：救命的是 `votes.length < 2` 那一行。存活 2 票或 3 票時，`> votes.length / 2` 跟原本的 `>= 2` 判斷完全一致（都要 2 票同意），兩者只在存活 4 票以上才分岔。比例寫法買到的是「票數浮動時門檻跟著動」，**不是**把「2 取 2」放寬回「3 取 2」——後者辦不到，2 票的多數本來就是 2。所以只剩 2 票時門檻反而比 3 取 2 更嚴，這是刻意的保守：寧可留在 `deferred`，也不要用半票放行。

**`unresolved` 跟 `rejected` 不能混為一談**——沒被否證跟被否證是兩件事。但兩者都必須留在 `seen` 裡：把 quorum 不足的移出 `seen` 會直接破壞 4.2 的收斂。正解是留在 `seen`（不再**重找**），另存 `deferred`，收工前**重判**一次（4.2 末段那幾行），只花裁決的 token，不用再跑 finder。重判後仍然 `unresolved` 的，列進報告的「未裁決」區並附上存活票數，不要當成 `rejected` 吞掉。
...
但**隔離只是必要條件**。harness §四 面向 5 已經寫過「分離是必要、不是充分」，[ihower-harness-engineering.md](./ihower-harness-engineering.md) §三 講得更直接：換新 context 仍共享同一套訓練、先驗與失誤模式，不算獨立檢查。

ihower §五 的「裁判獨立性光譜」是這件事在**單輪驗收**上的版本（自我審計 → 全新 context 的 grader agent）。它的結論是**取捨不是排名**——獨立性買得越多，裁判看得到的證據越少。
...
**第二層要錢**：node 並行寫檔會互踩，`isolation: 'worktree'` 給每個 agent 一份獨立工作副本。第一方契約把它標為 EXPENSIVE——每個 agent 約 200–500ms 建置加磁碟開銷，沒改動的 worktree 會自動移除。只有真的並行寫入才值得繳這筆。
...
直覺會想「把重複性 node 降到便宜模型」，但**第一方契約的預設剛好相反**：不設 `model`，除非你很有把握某個 tier 更適合（§八 #4）。

真正該先動的是另一根槓桿：**`effort`**（`low`／`medium`／`high`／`xhigh`／`max`）。機械性的 stage 調 `low`，只有最難的 verify／judge 往上加——**不換模型就能分層計價**，而且不會賭錯 tier。
...
開起來的方式不是「幾個並列的便利選項」，而是**授權**：Claude 不會主動開 workflow，一定要有人給它開的理由。合法授權共五種（完整清單見 §八 #14），常用的是這三種：
...
> **基準**：本機 Claude Code **v2.1.220** 內嵌的第一方 `Workflow` 工具契約與 runtime，2026-08-02 實測，方法與主要證據見 §十。原文寫於 2026-07-20。
>
> 以下多數是原文本來就沒寫的，不是它寫錯。另：#8、#12、#15 直接引自契約原文，未另立驗證列；#5 是概念校正，不屬於契約可驗證項。
...
- 失敗的 thunk 變 `null`
- `pipeline()` 無 barrier
- `schema` 在 tool-call 層驗證並重試
- `isolation: 'worktree'` 昂貴，只該用在平行寫檔
- loop-until-dry 要對 seen 去重
- 腳本是純 JavaScript

**只有「編排層零 model token」要降一格**（本篇正文刻意沒引用這個說法）。契約只寫「控制流應該是確定性的（迴圈、條件、fan-out），而不是模型驅動」。

契約裡確實談 token，但談的都是 `budget`（`+500k` 指令、`50_000`／`100_000` 這類門檻）與「workflow 會吃掉大量 token」的警告，**沒有任何一句給編排層本身的 token 成本下保證**。零 token 是從「腳本跑在 runtime 裡、不是又一輪對話」推出來的合理結論，不是契約寫死的。

### 需要修正或補精確度

#### #1 併發上限：`min(16, max(2, 核心數 - 2))`

> 原文：併發「大約是你的核心數」。

精確值是 **`min(16, max(2, CPU 核心數 - 2))`**，而且是**每個 workflow** 分別計。契約簡寫成 `min(16, 核心數 - 2)`，實作另有 2 的下限。

另有兩個硬上限：單一 workflow 生命週期**最多 1000 個 agent**（暴走保險）；單次 `parallel()`／`pipeline()` **最多 4096 個項目**，超過是明確報錯，不是無聲截斷。

#### #2 `ultracode` 同時把 effort 拉到 xhigh

> 原文：`ultracode` ＝「每個任務都規劃 workflow」。

少講一半。這句不在 Workflow 工具契約裡，而是 `ultracode` **設定項**的描述：「xhigh effort plus standing dynamic-workflow orchestration」——它**同時把 reasoning effort 拉到 xhigh**。（工具契約那邊只寫「opt-in 是常態性的，預設每個實質任務都寫一張圖」，沒提 effort。）

所以成本不只來自多開 agent，也來自每個 turn 變貴。它是 session-wide 的，重任務跑完記得關。

#### #3 存檔流程比按一個鍵長

> 原文：「按 `s` 存進 `.claude/workflows/`」。

按鍵屬實，但流程比原文完整：填**名稱** → **`tab` 切 scope** → `enter` 確認。兩種 scope 是 Project（存到 `.claude/workflows/<name>.js`）與 User（存到使用者層目錄）——**原文沒提 User scope**。

存完的提示明說它**同時是 slash command**：`/<name>` 或 `Workflow({name})`。同名會擋下並問覆寫。

#### #4 該先動的是 `effort`，不是 `model`

> 原文：「把重複性 node 降到便宜模型」。

方向對，但第一方預設相反：**不設 `model`**，「只在你很有把握某個 tier 更適合時才設；不確定就省略」。

而且原文沒提更該先動的那根槓桿：**`effort`**（`low`／`medium`／`high`／`xhigh`／`max`），不換模型就能分層計價（見 §六 6.2）。
...
原文自己舉的例子（C 卡住 D 不跑）講的是**串連耦合**，不是缺冗餘。圖買到的是獨立性與故障圍堵；冗餘要另外做（§一 1.2）。

#### #6 `null` 還有第二個來源：使用者 skip

> 原文：`agent()` 回 `null` ＝ agent 失敗。

還有第二個來源：**使用者中途 skip 掉那個 agent**。

**順帶修一條既有筆記**：[harness-engineering.md](./harness-engineering.md) §六 說 workflow「無中途使用者輸入」，精確講是**沒有輸入型的人工介入點**，但使用者仍可 skip 個別 agent——能中斷，不能引導。

### 原文完全沒提，但寫腳本會踩到

#### #7 `Date.now()`／`Math.random()`／無參數 `new Date()` 會 throw

原文的程式碼風格會誘導你順手用。

原因是 resume（#8）：時間戳記要在 workflow **回傳之後**再蓋，或從 `args` 傳進去；要隨機性就用 index 變化 prompt／label。

#### #8 Resume：`Workflow({scriptPath, resumeFromRunId})`

沒改動的 `agent()` 前綴直接回快取，只有第一個被改的呼叫及其之後才重跑。同腳本同 args ＝ 100% 命中。

這是**改腳本迭代的正確方式**，也是 #7 那些限制存在的理由。

#### #9 `budget` 全域：`total`／`spent()`／`remaining()`

原文警告 cycle 會燒光預算，卻沒說有內建煞車。使用者下「+500k」這類指令時 `total` 有值，而且是**硬上限**——到頂之後 `agent()` 直接 throw。

但**沒設目標時 `remaining()` 回 `Infinity`**，單靠它當守衛等於沒煞車，會一路撞到 1000 agent 天花板。runtime 的錯誤訊息自己就警告這件事，並要你加硬輪數上限。寫法見 §四 4.2。

#### #10 `pipeline()` 的 stage 收到 `(prevResult, originalItem, index)`

原文推薦 pipeline，卻沒說後段 stage 怎麼標記工作。有這三個參數就不必把 context 硬塞進 stage 1 的回傳值。

另外：某個 stage throw 只會把該項目降成 `null` 並跳過它剩下的 stage。

#### #11 `meta` 的必填欄位比你以為的少，但限制比你以為的多

必填只有 `name`／`description`，`phases`／`whenToUse` 選填。

`phases` 的 title 要跟 `phase()` 呼叫**逐字相同**才會併進同一個進度群組，對不上就自成一組。整個物件**不能有變數、函式呼叫、spread、模板字串插值**——違反了腳本解析不過。

#### #12 `workflow()` 可內嵌別的 workflow，但只有一層

子 workflow 共用同一個併發上限、agent 計數、abort signal 與 token 預算。子層再呼叫會 throw。

#### #13 除錯有 journal：`<transcriptDir>/journal.jsonl`

記錄每個 agent 的**實際回傳值**。workflow 回空結果要先讀它，別假設快取結果非空。

另有 `agent-<id>.jsonl` 逐 agent transcript。

#### #14 Workflow 是硬性 opt-in：共五種合法授權

1. prompt 裡出現 `ultracode`
2. **session 已開著 ultracode**（system-reminder 確認，與前者不同）
3. 使用者用自己的話要求（「use a workflow」「fan out agents」）
4. **skill 或 slash command** 的指示
5. 指名跑某個已存的 workflow

**「這個任務明顯很適合平行化」不構成理由**——契約明寫遇到這種情況要改成口頭描述可以怎麼跑、大概多少成本，然後問使用者。

#### #15 腳本是 JavaScript，不是 TypeScript

型別註記（`: string[]`）、interface、generics 一律解析失敗。

原文的 code fence 在 X 上被標成 `python`（渲染 artifact），內容其實是 JS。

#### #16 柔性上限：workflow size guideline

工具 prompt 每次附掛 `small`／`medium`／`large` ＝ 5／15／50 個 agent 的建議值，**預設 medium（15）**，措辭明寫「是建議不是硬上限」。可在 `/config` 的「Dynamic workflow size」調整或設成 unrestricted。

它跟 C、1000、4096 三個硬上限不同：**它影響的是 Claude 願意開多大的圖**。所以 §五 5.1 那種「20 個發現 × 3 個鏡頭 ＝ 60 agent」的規模，在預設 session 下有兩條路——在 prompt 裡明講要這個規模（契約自己留的例外），或先把 guideline 調寬。
...
| 主題                        | 本篇的位置                         | 既有筆記怎麼講                                                                                              |
| --------------------------- | ---------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Workflow primitive 選型     | 不談                               | harness §六 的四 primitive 表（sub-agent／skill／agent team／workflow）                                      |
| 同一個拓樸的別名            | fan-out／fan-in／diamond／pipeline | harness §四 面向 4 叫它「Fork-Join 並行」（`git worktree` 實作）                                             |
| 工具回傳值／node 契約       | §二 2.1 的 schema 硬化             | ihower §三「工具回傳值是寫給 agent 的回饋」                                                                  |
| Cycle                       | §四 4.2，run 內、狀態在腳本變數    | loop：跨 run、狀態在磁碟、心跳叫醒、`/goal` 判停                                                             |
| 裁判獨立性                  | §五 5.3：結構獨立、模型不獨立      | ihower §五 光譜（單輪驗收版，結論是取捨非排名）＋ harness §四 面向 5「分離是必要、不是充分」                 |
| 為什麼能開幾百個 agent 不爆 | §三 3.1 末段                       | harness §四 面向 1 tool-call offloading ＋ 該篇 §六「中間結果落點」                                          |
| 成本實感                    | §五 5.1 價錢欄、§六 6.2            | harness §六：workflow 單一重任務可吃掉 Max plan 約 20% session 限額；中小任務用 workflow 是純浪費            |
| 裁判本身準不準              | §五 5.1 只要求「prompt 偏向反駁」  | eval §一：判官的**家族偏誤**是正交的第二條軸                                                                 |

三處值得展開的接縫：

- **選型先於畫圖**：先看 [harness-engineering.md](./harness-engineering.md) §六 那張表決定要不要用 workflow（核心兩問：誰掌握計畫、中間結果落在哪；要 human-in-the-loop 就往表的左端走），再讀本篇決定圖怎麼畫。
- **offloading 的分工**：[context-engineering.md](./context-engineering.md) §2.3 只補 deferred tool loading，**offloading 那半它明講不重複**，要看 harness。
- **judge panel 在兩篇不同義**：[eval-engineering.md](./eval-engineering.md) 替本篇 §五 5.3「裁判是動 `model` 的唯一例外」補上計量證據，它建議的跨廠商 judge panel 要連同本篇 §五 5.2 的 quorum 陷阱一起讀——但注意本篇把 judge panel 歸為**選拔**，該篇是拿它當 **gate** 用。
...
原文是一則 X 長貼文，中段夾作者 Substack 導流。**它是二手材料**——內容是 Anthropic dynamic workflows primitive 的教學化重述。

價值在於它的**提法**：「edge 的判準是資料有沒有跨過去」這個切法官方文件沒有，而它確實能改變你讀自己腳本的方式。

但**技術細節一律以第一方為準**。本篇把原文的每個技術主張都拿去對 v2.1.220 的工具契約，結果集中在 §八。原文的收尾一句值得留著：「A prompter asks a question. An architect draws a graph.」（Codez, 2026-07-20）

### 14 步 → 本篇章節

**這張表按校準重要性排，不按 step 序。** 粗體的 #1、#4、#14 是原文站不住或講反的地方；#2 與 #3 性質不同——說法本身屬實，只是各漏了一半（#2 漏掉 effort 被一起拉到 xhigh，#3 漏掉存檔流程與 User scope）。
...
- **2026-08-04（可讀性）**：**技術主張、數字與校準結論一條未動**，改的是承載方式。§八 兩張大表拆成 `#### #1`～`#### #16` 十六個小標（原本 20 個儲存格超過 120 顯示寬，最寬 411，等於把論證段落塞進表格）；§九 三格過長的儲存格改為表格＋三條展開說明。表格佔正文比例 58.2% → 33.5%，最寬儲存格 411 → 97
  - §四 4.2 原本在程式碼裡呼叫 §五 5.2 才定義的 `judgeAll`，而 5.2 又回指 4.2——改成先給 `judgeAll` 的契約（收一批發現、回傳帶 `verdict`、無 `null`），把互相依賴降為單向前向指標
  - §五 5.2 原本用三段講同一個結論，合併為兩段（刪掉「兩個邊界先講死」那段的重複部分，保留其中「刻意保守」與「未裁決要附存活票數」兩點）
  - 正文的「原文說 X，其實是 Y」改寫成直接下結論，裁決留在 §八（§一 1.2、§三 3.1、§六 6.2、§七）；前言的「來源該怎麼看」與 14 步對照表下沉到 §十
  - 修三處會把讀者送錯地方的 `§六`：原文指的是 harness §六，但本篇也有 §六（§前言術語接縫、§三 3.1 註、§九）
